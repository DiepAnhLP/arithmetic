...
object ArithmeticExpression {
  def evaluate(expression: ArithmeticExpression): Double = expression match {
    ...
    case Pow(base, exponent) if evaluate(exponent) > 0
      => evaluate(base) * Math.pow(evaluate(base), evaluate(exponent).doubleValue.toInt - 1)
    case Pow(base, exponent) if evaluate(exponent) < 0
      => (1 / evaluate(base)) * Math.pow(evaluate(base), evaluate(exponent).doubleValue.toInt + 1)
  }
